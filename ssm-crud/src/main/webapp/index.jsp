        ...
        <script type="text/javascript">
            ...
            function build_emps_table(result) {
                ...
                $.each(emps,function (index, item) {
                    ...
                    // 所以<tr>每次追加后还是返回<tr>，再继续append填内容就行
            ...
            // 3、解析并显示分页条数据，并且点击分页页码要能超链接到对应页-----------------------
            function build_page_nav(result) {
                ...
                // 如果没有前一页和首页，则不能点击
                if(result.extend.pageInfo.hasPreviousPage == false){  // 当前页没有上一页，则当前页就是第一页
                ...
                if(result.extend.pageInfo.hasNextPage == false){  // 当前页没有下一页，则当前页就是最后一页
                    nextPageLi.addClass("disabled");  // 禁用
                ...
                $.each(result.extend.pageInfo.navigatepageNums,function (index,item) {
                    ...
                    // 如果当前页码等于正在遍历的页码，增加一个被选中的active标识
            ...
            $("#emp_save_btn").click(function () {
                ...
                $.ajax({
                    url:"${APP_PATH}/emp",
                    type:"POST",
                    data:$("#empAddModal form").serialize(),  // 表格数据序列化
                    success:function (result) {
                        // alert(result.msg);
                        if(result.code == 100){
                            // 当员工添加保存成功时：
            ...
            //编辑 按钮的更新事件----------------------------------------------------------------------------------
            //为每个员工的编辑按钮绑定单击事件
            //但是，这里的单击事件click是在编辑按钮被创建出来之前就已经开始绑定了，所以这里无法绑定单击事件
            //有两种办法
